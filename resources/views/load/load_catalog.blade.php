...
@foreach ($data as $product)
    ...
    <div class="col-xxl-3 col-xl-4 col-lg-6 col-md-4 col-sm-6 pb-4">
        <!-- BEGIN card -->
        <div class="card open-modal" data-md_nameproduct="{{ $product->produk }}" data-image_product="{{ $image_product }}"
            data-md_warehouse="{{ $product->warehouse[0]['warehouse'] }}" data-md_price="{{ $image_product }}"
            data-id_produk="{{ $product->id_produk }}" data-id_area="{{ $product->id_area }}"
            data-id_brand="{{ $product->brand }}" data-quality="{{ $product->quality }}"
            data-n_price="{{ $product->n_price }}" data-r_price="{{ $product->r_price }}"
            ...
            <div class="">
                <div style="font-size: 10px;margin-bottom:10px;margin-top:5px;text-align:center;">
                    {{ $product->produk }}</div>
            </div>
        </div>
